...
The engine uses an alpha-beta search with iterative deepening as its main algorithm.
  ...
  - **Time management (v0.4.1).** A *soft* budget (`softTimeLimit = remaining / (21 + pieces*2/3)`) decides when to stop *starting* new iterations; an *easy-move* early stop (best move stable for several plies) is gated on spending ≥50% of it. A *hard* per-move cap (`maxTimeLimit = ourTime − max(50ms, 5%)`, set in `THEngine::goSearch`) is polled mid-search by the main thread every 2048 nodes (`alphaBetaSearch`/`quiesenceSearch` read `threads.stop`); when hit, the search unwinds and `iterativeSearch` falls back to the last fully-completed iteration's move. The hard cap is the safety net that makes the soft budget aggressive without ever flagging.
  ...
  - **Formula-based LMR (v0.3.14).** The conservative `R = 1|2` reduction was replaced by a Stockfish-style depth×move-count log formula: `Reductions[i] ≈ 23·log(i)` (`src/worker.cpp:14`), `R = Reductions[depth]·Reductions[movesSearched] / LMR_SCALE` (`LMR_SCALE = 1024`), nudged by butterfly history (`R -= historyScore/8000`, ±~2 plies), applied to quiet non-checking moves at `depth ≥ 2`, `movesSearched ≥ 2`, with a zero-window reduced search and a full-depth re-search on fail-high. Marginal on its own (**+5.4 ± 32.2 Elo**, very draw-heavy) but the EBF harness confirmed **~24 % fewer nodes to depth 13** (mean EBF 3.30 → 3.24); kept as the substrate the pruning suite needs.
  - **SEE pruning in the main search (v0.3.16).** The quiescence-only SEE filter was extended into the main tree: at `depth ≤ 6`, capture-stage moves whose static exchange evaluation is worse than `−75·depth` are skipped before being made (`src/worker.cpp:327`), guarded by `value > −29000`. Conservative (captures only — it discards clearly material-losing captures, which almost never matter) because the modest eval made aggressive lossy pruning risky. (v0.3.15 was a late-move-pruning attempt — a clear −49 Elo regression, not released.) **+10.9 Elo** vs 0.3.14.
  - **Per-node static eval + reverse futility (v0.3.17).** The static eval is now computed once per non-check node (`src/worker.cpp:231`) and reused as both the null-move gate and the reverse-futility test: at `depth ≤ 3`, when the static eval clears beta by `175·depth` eval-units and is not near-winning (`< 20000`), the node returns without searching. Deliberately conservative — the engine was not yet PVS, so eval-pruning full-window nodes is risky (an aggressive margin broke mate + tactics). **+5.4 Elo**, and the enabler the forward-pruning steps build on.
  - **Principal Variation Search (v0.3.18).** The first move at a node is searched full depth + full window; every later move is searched first with a zero (null) window — formula-LMR-reduced for late quiet non-checking moves — and re-searched at full depth/window only on a fail-high (`src/worker.cpp`, both the not-in-check and in-check loops). Value-exact (same tree, cheaper scouts). Neutral on the broken-TT engine, a clear gain on the lean v0.3.17 tree, and — crucially — it creates the zero-window (non-PV) nodes where eval-based forward pruning is safe. **+16.3 Elo** vs 0.3.17.
  - **Cross-entropy eval net (v0.4.5, `n512_h14x16_ce`).** The current release net. Same search as
    v0.4.4; the whole delta is the net. **+61 ± 29 Elo (bullet 1+1) and +72 ± 27 (blitz 3+2)** over
    v0.4.4, 200 games per control. It came out of famG, where 13 arms trained 300 epochs varying
    *only the loss function*: cross entropy beat MSE-2.5 in **11 of 11 head-to-head crosses**
    (660 games, +73 Elo). Mechanism: MSE-p penalises `|t − pred|^p` with `t ∈ [0,1]`, which confines
    the output to the box *and* whose gradient dies at the border (decays as saturation^p); CE's
    `clamp` leaves zero gradient outside, so nothing punishes overshoot. The engine reads the raw
    output as `eval = 4096·p − 2048`, so that box **is** the eval's dynamic range — CE nets carry
    2.5× more eval in the linear PSQT lane (psqt rms 470-703 vs 146-331, perfect separation over 24
    nets). ⚠️ The offline metric in use (**pearson on |cp|≤50**) ranked the CE arms *4th and 7th of
    10* — it predicted the board result backwards. The band that separates them is `all`, and the
    cleanest statistic is the jump `pearson(all) − pearson(1000)`, which classifies CE/non-CE with
    zero errors; `spearman(all)` is flat across all twelve recipes, so every recipe *orders*
    equally well and CE is the only one that gets the **scale** right.
  - **N512 eval net (v0.4.0) + time management (v0.4.1).** Covered under *Position Evaluation* and *Time management* above; together **+138 Elo** over 0.3.18. v0.4.1 was the current release on `threading`; the release line has since moved to `famF/ft-phase`, which carries v0.4.3, v0.4.4 and **v0.4.5 (current)**. `threading` is strictly behind it and fast-forwards cleanly.
  - **Continuation history (v0.4.2 candidate — rejected, neutral).** A `[prevPiece][prevTo][curPiece][curTo]` table added to quiet move ordering (in `select_legal`'s lazy stage 2, where quiets are actually scored — *not* `score()`, which is dead for this selector). It reaches **~1 ply deeper on average** (a genuinely more efficient search) but is **Elo-neutral** (−10.9 ± 40.1 over 64 games, 78 % draws): the existing ordering already keeps the PV stable, so the extra depth just re-confirms the same move. Not merged — same eval-bound pattern as QS-delta / TT-clustering. The infrastructure is kept for a possible pruning-input reuse (contHist + an `improving` flag feeding LMR / futility).
...
> **STATUS (v0.4.1 — read this first).** Most of the levers below are **done**; they *are* the v0.3.7–v0.4.1 release history, kept here as the historical record:
> TT best-move + correct bounds (task 1), staged/lazy picker (task 2), butterfly-history ordering (task 3), cheap legality (task 4), formula LMR (v0.3.14), main-search SEE pruning (v0.3.16), per-node static eval + reverse futility (v0.3.17), PVS (v0.3.18), and the **eval frontier** — dropping the fused `firstW2Indices` table and widening the accumulator to 512 (the **N512_h32** net, v0.4.0) — plus the v0.4.1 time-management fix.
> **Still open:** the forward-pruning suite Stockfish has and we don't — **futility, LMP, history pruning, razoring** (see the *pruning & timing profile vs Stockfish* tables in the Search section; gated on whether the N512 eval is now accurate enough for lossy pruning, since every prior attempt on the small eval failed); **aspiration windows**; **N512 eval-speed** (the ~6.5× nps cost is the main eval follow-up); **lazy-SMP multithreading**; and **piece-count output buckets**. Continuation-history ordering was tried (v0.4.2 candidate) and came back Elo-neutral.
...
**Priority (impact × ease).** The search levers **(1) TT best-move + bound → (2) lazy/staged picker → (3) real ordering → (4) cheap legality** are the highest-ROI and are independent of the eval. The eval track **(drop fused-diff table → widen accumulator → output buckets)** is the route to a *stronger* evaluation when you want it (the eval was then only ~12 % of search time, so it had headroom — *note: post-N512 the eval is now ~87 %; see the profile table in the Search section*). The three bullets immediately below are the **profiling evidence** that justifies these priorities; the numbered **tasks** follow.
...
- **Perft isolation — is the cost the generator or the picker? (startpos depth 6 = 119 M nodes, single-thread M2).** Original perft **62 Mnps** → without the eager `score()`+`sort_moves()` in `init_all` **78 Mnps** → also without the debug-string building (`perft_recursive` builds `prefix + move.toString() + " "` even when `outfile==nullptr`, `src/engine.cpp:160`) **95 Mnps**; Stockfish `go perft` ≈ **350 Mnps**. Conclusions: (1) the **move generator is fine** — ~95 Mnps clean, only ~3.7× behind SF (headroom, not a crisis), *not* the 4.7 Mnps an old note implied; (2) the **eager picker is the real search cost** — it costs ~20 % even in perft (where ordering is useless) and ~28 % in search (where early cutoffs make front-loaded scoring+sorting mostly wasted), so going staged/lazy is the win; (3) side-finding: the **perft benchmark wastes ~22 % building debug strings** — guard the `prefix + …toString()` concatenations with `if (outfile)` so the NPS regression numbers are honest (does not affect play, since perft is not on the search path).
  ...
  - **The "4× slower than Stockfish" was mostly a benchmark artifact (2026-06-04, M2, same machine, after the v0.3.7 cheap-legality fix).** Our `perft_perf` test times `THEngine::perft_recursive`, which is **search-shaped** — it does a **TT probe + TT save and a full `score()`+`sort_moves()` per node** (`src/engine.cpp`), none of which perft needs and none of which SF's `go perft` does. A lean `pure_perft` (generate + inline legality + make/unmake only, **identical node counts**) runs **2.56× faster: 85M → 217M nps** (depth 5). Stockfish on the *same* machine, startup-cancelled via the `go perft 7` − `go perft 6` difference (its 79 MB net costs ~0.6 s to load, which swamps a single `go perft 6`), is **≈360 Mnps** — so the doc's "~350" is right but the *fair* raw-machinery gap is **only ~1.66×** (217M vs 360M), not ~4×. The earlier "~41 % legality" self-time was inflated by that search overhead; after the v0.3.7 legality fix the `pure_perft` self-time profile is **~49 % the perft loop/recursion/per-move-legality-dispatch, ~31 % the six `*AllMoves` generators, ~6 % `newKingSquareIsSafe`, ~4.3 % `makeMove` (board + zobrist + check-detection + NNUE record — of which the NNUE `NNUEUChange::add` is only ~0.38 %), ~3 % `setBlockersAndPinsInAB`, ~2.3 % `unmakeMove`**. Takeaways: (1) the **raw generators are genuinely competitive** (~1.66× off SF, diffuse — no single bottleneck); SF's remaining edge is structural — one fused `generate<LEGAL>` pass vs LaMano's six generator calls + a separate per-node pin pass + a per-move legality test, plus a 4-byte `ExtMove` vs LaMano's 8-byte `ScoredMove` and a tighter perft loop; (2) **`makeMove`/NNUE is not the bottleneck** (4.3 % total, NNUE record 0.38 %); (3) **our perft dashboard under-reports raw move-gen speed by 2.56×** — to track generation honestly, benchmark a TT-less, sort-less `pure_perft`, not `perft_recursive`.
...
1. **Store the best move in the TT, and store a correct bound** — *one-line change, large gain; prerequisite for tasks 2–3.*
   ...
   - **✅ Implemented.** `best_move` is assigned on every alpha improvement and stored in the TT; bounds are stored correctly (`BOUND_LOWER` on fail-high, `BOUND_UPPER` on fail-low, `BOUND_EXACT` only when alpha is raised strictly inside the window) — `src/worker.cpp` + `src/ttable.h`. The `cntTTcut` counter confirms the signal is live (~11 cuts / 1000 nodes; see the profile table above). This unlocked tasks 2–3 and was a large part of the v0.3.13 TT-fix gain.
...
2. **Lazy / staged move picking in `ABMoveSelectorNotCheck`** — *the single biggest NPS lever.*
   ...
   - **✅ Implemented (v0.3.7 candidate, commit `b5b872e`).** `ABMoveSelectorNotCheck` is now a two-stage picker: `init_all` generates + AB-scores + sorts **only captures + queen promotions**; the quiets are generated, scored (killers boosted) and sorted **lazily in `select_legal`**, and only if no capture caused a beta cutoff. The TT move is still searched before the selector, so it is skipped here. The capture/quiet split reuses the existing `{pawnCapturesAndQueenProms, knight..king}Captures` generators for stage 1 and the all-moves generators for stage 2, dropping the repeats with the new `BitPosition::isCaptureStageMove` (a non-promo capture or a queen promotion). Notes: (a) a *first* attempt — replacing the eager `sort_moves` with QS-style lazy *find-best-remaining* over the full mixed list — **regressed** both perft (−62 %) and tactics (+2 %): with no cutoffs that loop is a full O(n²) selection sweep with a worse constant than the one-shot insertion sort, so the win is **not** a cheaper sort but **not generating the quiets at all** on cut nodes; (b) the staged version keeps the eager insertion sort, applied to the small per-stage lists. Measured (clean-HEAD worktrees, M2; perft node counts identical, all ctest green): **deep fixed-depth tactics −27.5 % (99.0 s → 71.8 s)**, perft-AB NPS ~neutral (−0.9 %; the only extra work on a no-cutoff node is regenerating the few captures). Not yet done from the original plan: splitting captures good/bad with `see_ge` (ii), and dedicated quiet-only generators to avoid re-generating captures in stage 2 (a further small NPS opportunity on no-cutoff nodes).
...
3. **Real move ordering for the AB selectors** — *highest ROI together with tasks 1–2.*
   ...
   - **✅ Implemented (v0.3.12).** Captures scored by `aBMoveValue` (MVV-LVA-style), then killers, then the butterfly history `mainHistory[stm][from][to]` for quiets (in the staged selector). **Continuation history** was added next (v0.4.2 candidate) but came back **Elo-neutral** (see the Search section) — capture-history is still unimplemented.
...
- **✅ Principal Variation Search (PVS) — done (v0.3.18).** The first move uses the full `(-beta, -alpha)` window; later moves a null window `(-alpha-1, -alpha)`, re-searched at full window only on fail-high. As predicted it paid off once ordering + TT + LMR + SEE were in place, and it creates the non-PV nodes where eval-based forward pruning is safe.
- **✅ SEE pruning in the main search — done (v0.3.16).** `alphaBetaSearch` skips capture-stage moves with `see_ge(move, −75·depth)` at depth ≤ 6 (`src/worker.cpp:327`). Pruning *quiets* by SEE is still open.
- **⏳ Aspiration windows (open).** Search the root with a narrow window around the previous iteration's score and widen on fail-high/low, instead of the current ±31000 full window.
- **✅ Late Move Reductions (LMR) in the main search — done (v0.3.11, formula v0.3.14).** Late quiet non-checking moves are reduced by `Reductions[depth]·Reductions[movesSearched]/1024`, history-nudged, with a full-depth re-search on fail-high. The `improving`-flag refinement is **still open** — it broke the b2d4 tactic in trials, so it belongs with a futility/LMP consumer, not as a bare reduction.
- **⏳ Lazy SMP multithreading (open).** The `ThreadPool` framework exists but only thread 0 searches. Run N worker threads sharing the global transposition table.
- **Faster NNUEU evaluation.** *(Re-framed post-N512 — the old "~12 %" no longer holds.)* Since v0.4.0 the eval is the **dominant** cost: the N512 forward pass is **~69 %** of self-time and accumulation **~18 %** (see the profile table in the Search section), so eval-speed is now the **#1 open speed lever**. **Done:** SIMD (NEON **SDOT** `vdotq_s32`) for the accumulator / affine layers; and the king-agnostic accumulator (the NNUEU "Ultra" trick) already avoids the king-move accumulator refreshes that cost Stockfish its Finny-table machinery (~4 %). **Open:** reduce the N512 forward-pass cost (the inherent ~6.5× nps vs w32 — the head is wide relative to the accumulator, so a narrower head / fewer dense MACs are candidates). Note: exploiting the ~80 %-zero accumulator with **sparse-input affine** is an x86/AVX-VNNI win *only* — on NEON it measured ~2× **slower** than dense `vdotq` (4 MACs/instr), so it is not a lever here.
  - **✅ Done (v0.4.0): dropped the `firstW2Indices` / `firstW2IndicesInv` fused-difference tables** (~838 MB at width 512). `addAndRemoveOnInput` now does add+remove via `firstW`/`firstWInv` (mathematically identical) — this was the prerequisite that unblocked the widening below. *Original note (for context):* A non-king move fuses "remove from-square + add to-square" into one vector add by looking up a precomputed `firstW[i] - firstW[j]` in `firstW2Indices[640][640][FIRST_OUT]` (declared in `src/accumulation.h`, built in `Transformer::load`, `src/accumulation.cpp:421`, used in `addAndRemoveOnInput`, `src/accumulation.cpp:160`). The naive alternative — `addOnInput(to)` + `removeOnInput(from)`, both already implemented (`src/accumulation.cpp:209`) — touches only the ~10 KB `firstW`/`firstWInv` tables (L1-resident) instead of the ~13 MB pair. Action: replace the lookup in `addAndRemoveOnInput` with the two ops and delete the tables + their build loop.
...
- **✅ Done (v0.3.17).** If not in check, a static eval is computed once per node (`src/worker.cpp:231`), enabling reverse-futility and the null-move gate. Forward *futility* on top of it is still open (see the pruning suite).
