...
- Replace the QS perft test with a direct "QS captures vs AB-legal captures" check and remove the test-only non-capture QS generators. The QS perft currently validates the QS machinery by generating *all* moves (captures + non-captures) and comparing to the full perft, which requires a large set of non-capture generators in `bitposition.cpp/.h` (`*NonCaptures`, `inCheck*Blocks`, ...) used only by the test-only `QSMoveSelector*NonCaptures` selectors. The real quiescence search only ever generates captures, and those were verified correct at depth 5; the only QS-perft discrepancy is an under-promotion bug in the test-only `inCheckPawnCapturesNonQueenProms` path (no effect on play). So the cleaner design is: walk the AB tree and assert that the QS capture selectors produce exactly the AB legal moves that are captures or queen-promotions (reference counts up to depth 5), then delete the test-only non-capture generators to shrink the codebase.
