...
TEST_CASE("Tactic 1 Time-Limited", "[tactics][time-limited]") {
    #ifdef NDEBUG
        run_time_limited_tactic_test("1", "kbK5/pp6/1P6/8/8/8/8/R7 w - - 0 1", "a1a6", 1000);
    #endif
}

TEST_CASE("Tactic 2 Time-Limited", "[tactics][time-limited]") {
    #ifdef NDEBUG
        run_time_limited_tactic_test("2", "rR6/p7/KnPk4/P7/8/8/8/8 w - - 0 1", "c6c7", 1000);
    #endif
}

TEST_CASE("Tactic 3 Time-Limited", "[tactics][time-limited]") {
    #ifdef NDEBUG
        run_time_limited_tactic_test("3", "1b1q4/8/P2p4/1N1Pp2p/5P1k/7P/1B1P3K/8 w - - 0 1", "b2d4", 1000);
    #endif
}

TEST_CASE("Tactic 4 Time-Limited", "[tactics][time-limited]") {
    #ifdef NDEBUG
        run_time_limited_tactic_test("4", "2r2rk1/1b3ppp/p1qpp3/1P6/1Pn1P2b/2NB1P1P/1BP1R1P1/R2Q2K1 b - - 0 19", "c6b6", 1000);
    #endif
}

TEST_CASE("Tactic 5 Time-Limited", "[tactics][time-limited]") {
    #ifdef NDEBUG
        run_time_limited_tactic_test("5", "rn2kb1r/1bq2pp1/pp3n1p/4p3/2PQ1B1P/2N3P1/PP2PPB1/2KR3R w kq - 0 12", "f4e5", 1000);
    #endif
}

TEST_CASE("Tactic 6 Time-Limited", "[tactics][time-limited]") {
    #ifdef NDEBUG
        run_time_limited_tactic_test("6", "3k2rr/4b3/p3Qpq1/P2pn3/1p1Nb3/6B1/1PP1B2P/3R1RK1 b - - 0 25", "h8h2", 1000);
    #endif
}

TEST_CASE("Tactic 7 Time-Limited", "[tactics][time-limited]") {
    #ifdef NDEBUG
        run_time_limited_tactic_test("7", "4k3/Q6n/8/8/8/8/PR5P/4K1NR w K - 0 1", "b2b8", 1000);
    #endif
}
