...
// (by clipping to max int8) and clip negatives to zero before each layer pass.
{
    ...
    // ********** Print Layer 1 input **********
    printf("========== Layer 1 ==========\n");
    printf("Input (vector) - 8 elements:\n");
    {
        int8_t debug_vector[8];
        vst1_s8(debug_vector, vector);
        for (int i = 0; i < 8; ++i)
        {
            printf("%d ", debug_vector[i]);
        }
        printf("\n\n");
    }
    ...
    // ********** Print Layer 1 weights (8x8) **********
    printf("Weights1 (8x8):\n");
    for (int i = 0; i < 8; ++i)
    {
        int8_t debug_w[8];
        vst1_s8(debug_w, weight1[i]);
        for (int j = 0; j < 8; ++j)
        {
            printf("%d ", debug_w[j]);
        }
    ...
    int16x8_t bias1 = vld1q_s16(secondLayerBiases);

    // ********** Print Layer 1 biases (8 elements) before addition **********
    printf("Bias1 (8 elements):\n");
    {
        int16_t debug_b1[8];
        vst1q_s16(debug_b1, bias1);
        for (int i = 0; i < 8; ++i)
        {
            printf("%d ", debug_b1[i]);
        }
    ...
    // ********** Print Layer 1 output (8 elements) **********
    printf("Output1 (8 elements):\n");
    {
        int16_t debug_out1[8];
        vst1q_s16(debug_out1, output1);
        for (int i = 0; i < 8; ++i)
        {
            printf("%d ", debug_out1[i]);
        }
        printf("\n\n");
    }
    ...
    // ********** Print Layer 2 input (8 elements) **********
    printf("========== Layer 2 ==========\n");
    printf("Input2 (8 elements):\n");
    {
        int8_t debug_in2[8];
        vst1_s8(debug_in2, input2);
        for (int i = 0; i < 8; ++i)
        {
            printf("%d ", debug_in2[i]);
        }
        printf("\n\n");
    }
    ...
    for (int i = 0; i < 4; ++i)
    {
        weight2[i] = vld1_s8(thirdLayerWeights + i * 8);
    }

    // ********** Print Layer 2 weights (4x8) **********
    printf("Weights2 (4x8):\n");
    for (int i = 0; i < 4; ++i)
    {
        int8_t debug_w2[8];
        vst1_s8(debug_w2, weight2[i]);
        for (int j = 0; j < 8; ++j)
        {
            printf("%d ", debug_w2[j]);
        }
    ...
    int16x4_t bias2 = vld1_s16(thirdLayerBiases);

    // ********** Print Layer 2 biases (4 elements) before addition **********
    printf("Bias2 (4 elements):\n");
    {
        int16_t debug_b2[4];
        vst1_s16(debug_b2, bias2);
        for (int i = 0; i < 4; ++i)
        {
            printf("%d ", debug_b2[i]);
        }
    ...
    // ********** Print Layer 2 output (4 elements) **********
    printf("Output2 (4 elements):\n");
    {
        int16_t debug_out2[4];
        vst1_s16(debug_out2, output2);
        for (int i = 0; i < 4; ++i)
        {
            printf("%d ", debug_out2[i]);
        }
        printf("\n\n");
    }
    ...
    // ********** Print Layer 3 input (8 elements, though effectively first 4 are valid) **********
    printf("========== Layer 3 ==========\n");
    printf("Input3 (8 elements, first 4 used):\n");
    {
        int8_t debug_in3[8];
        vst1_s8(debug_in3, input3);
        for (int i = 0; i < 8; ++i)
        {
            printf("%d ", debug_in3[i]);
        }
        printf("\n\n");
    }

    // Load only 4 bytes and zero-pad safely
    int8x8_t weight3 = vld1_s8(finalLayerWeights);

    // ********** Print Layer 3 weights (8 elements, first 4 used) **********
    printf("Weights3 (8 elements, first 4 used):\n");
    {
        int8_t debug_w3[8];
        vst1_s8(debug_w3, weight3);
        for (int i = 0; i < 8; ++i)
        {
            printf("%d ", debug_w3[i]);
        }
    ...
    // ********** Print finalLayerBias before addition **********
    printf("finalLayerBias: %d\n\n", finalLayerBias);

    int16_t output3 = vaddvq_s16(vmull_s8(input3, weight3)) + finalLayerBias;

    // ********** Print final output (single value) **********
    printf("Output3 (single value): %d\n", output3);
