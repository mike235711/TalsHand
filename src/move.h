...
public:
    ...
    explicit Move(unsigned short origin, unsigned short destination, unsigned short moving_piece, unsigned short promotion_flag) : // Normal capture constructor (Non promotion, capture, castling or double pawn move)
            data{static_cast<uint16_t>(origin | (destination << 6) | (moving_piece << 12) | (promotion_flag << 15))}
    {
    }
    ...
    unsigned short getMovingPiece() const
    // Only for moves generated by orderedCaptures
    {
        return (data >> 12) & 7;
    }
    unsigned short isPromotion() const
    // Only for moves generated by orderedCaptures
    {
        return data >> 15;
    }
