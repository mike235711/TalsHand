...
namespace NNUEU
{
...

    {
        ...
        for (int n = 0; n < 4; ++n)
        {
            const int8_t *w = weights.thirdW + n * 8;
            int32_t acc = 0;
            for (int k = 0; k < 8; ++k)
                acc += static_cast<int32_t>(l1_int8[k]) * static_cast<int32_t>(w[k]);

            acc += static_cast<int32_t>(weights.thirdBias[n]);
        ...
        int32_t acc = 0;
        for (int k = 0; k < 8; ++k)
            acc += static_cast<int32_t>(l2_int8[k]) * static_cast<int32_t>(weights.finalW[k]);

        acc += static_cast<int32_t>(weights.finalBias);
